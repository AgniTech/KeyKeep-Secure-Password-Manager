...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const { masterPassword } = req.body; // Changed from 'password' to 'masterPassword' for clarity
    if (!masterPassword) {
    ...
    const isAuth = await user.comparePassword(masterPassword); // Use masterPassword
    if (!isAuth) {
      return res.status(401).json({ error: 'Invalid master password.' });
    }

    const argon2Salt = Buffer.from(user.argon2Salt, 'base64');
    PKEK = await argon2.hash(masterPassword, { // Use masterPassword
        salt: argon2Salt,
        raw: true,
    ...
    res.status(200).json({ credentials: decryptedEntries }); // Wrap in 'credentials' object
